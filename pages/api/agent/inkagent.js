...
const BARE_DOMAIN_REGEX = /\b(?:[\w-]+\.)+(?:com|dev|org|io|net|co|app|blog)\b/gi;
...
function extractAndValidateUrls(text) {
	...
	const fullUrls = (text.match(URL_REGEX) || []).map((u) => u.replace(/[.,;:!?]+$/, ""));
	...
	if (bareMatch && bareMatch.length > 0) {
		return { valid: false, error: "URLs must include https:// (e.g. https://example.com)" };
	}
...
export default async function handler(req, res) {
	...
	if (!urlCheck.valid) {
		return res.status(400).json({ error: urlCheck.error || "Invalid URL. Use full URLs with https://" });
	}
