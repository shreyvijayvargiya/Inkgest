	...
	try {
		...
		const processNode = (node) => {
			...
			if (tag === "figure" && node.getAttribute("data-block") === "audio-block") {
				...
				const wavezincE = `repeating-linear-gradient(90deg,#C17B2F 0,#C17B2F 2px,transparent 2px,transparent 8px)`;
						...
						`<div style="background:#37352F;border-radius:10px;padding:10px 14px;display:flex;align-items:center;gap:12px">` +
							...
							`<div style="flex:1;display:flex;flex-direction:column;gap:5px;min-width:0">` +
								`<div onclick="${seekJs}" style="position:relative;height:20px;cursor:pointer;display:flex;align-items:center">` +
									...
									`<div data-ap style="position:absolute;left:0;top:0;height:100%;width:0%;overflow:hidden;pointer-events:none"><div style="position:absolute;inset:0;display:flex;align-items:center"><div style="width:9999px;height:3px;background:${wavezincE};border-radius:2px"></div></div></div>` +
								`</div>` +
