	...
	try {
		...
		const processNode = (node) => {
			...
			if (
				...
				return `<div data-ink-mermaid-wrap="" class="ink-mermaid-slot" style="margin:20px auto;max-width:100%;overflow-x:auto;text-align:center"><pre class="mermaid" data-ink-mermaid="1">${escapePcdata(raw)}</pre></div>\n`;
			}
			if (tag === "pre")
			...
			if (tag === "div" && node.getAttribute("data-ink-mermaid-wrap") != null) {
				...
				const capPart = cap
					...
					: "";
				return `<div data-ink-mermaid-wrap="" class="ink-mermaid-slot" style="margin:22px auto;max-width:100%;overflow-x:auto">${capPart}<pre class="mermaid" data-ink-mermaid="1">${escapePcdata(raw)}</pre></div>\n`;
			}
