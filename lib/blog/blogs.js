/**
 * Blog registry — add a row and a matching file under content/blog/ to publish.
 * `file` is the filename inside content/blog/; body markdown is read at build time.
 */
export const BLOG_POSTS = [
	{
		slug: "inkgest-mcp-server-claude-desktop",
		title: "Claude, meet your Inkgest library — the MCP server is live",
		description:
			"Connect Claude Desktop to Inkgest in minutes: search drafts, create and update posts, manage your writing board, and save translations — without leaving the chat.",
		banner: "/inkgest-logo.png",
		tags: ["MCP", "Claude", "Integrations", "Product"],
		file: "inkgest-mcp-server-claude-desktop.md",
		publishedAt: "2026-06-21",
	},
	{
		slug: "inkgest-for-creating-stunning-newsletter-emails",
		title: "Inkgest for creating stunning newsletter emails",
		description:
			"How AI streamlines newsletter creation, from structured drafts to stronger CTAs—plus practical tips your subscribers will notice.",
		banner: "/inkgest-logo.png",
		tags: ["Newsletter", "AI", "Product"],
		file: "inkgest-for-creating-stunning-newsletter-emails.md",
		publishedAt: "2026-03-15",
	},
	{
		slug: "getting-started-with-inkgest",
		title: "Getting started with Inkgest in under ten minutes",
		description:
			"Paste a URL, describe your angle, and ship a first draft—what to expect from the editor and how to iterate fast.",
		banner: "/inkgest-logo.png",
		tags: ["Guide", "Product"],
		file: "getting-started-with-inkgest.md",
		publishedAt: "2026-03-20",
	},
	{
		slug: "scrape-rss-feeds-into-tables-with-inkgest",
		title:
			"From RSS directories to tables: scraping FeedSpot & frontend-feeds with Inkgest",
		description:
			"Turn curated programming and front-end RSS listings into structured tables—use cases for newsletters, research, and how FE/BE developers can use the feeds.",
		banner: "📡",
		tags: ["Table", "RSS", "Scrape", "Guide"],
		file: "scrape-rss-feeds-into-tables-with-inkgest.md",
		publishedAt: "2026-04-01",
	},
];
