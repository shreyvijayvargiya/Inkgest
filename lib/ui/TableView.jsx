...
function EditableCell({ value, type, rowIdx, colKey, onSave }) {
...
export default function TableView({ tableId, tableData, setTableData, reduxUser }) {
	...
	const handleSaveCell = useCallback(
		async (rowIdx, colKey, newVal) => {
			if (!tableData || savingCell || !tableId) return;
			setSavingCell(true);
	...
	const handleAddColumn = useCallback(() => {
		if (!tableId) return;
		let key =
			(newColKey || newColLabel || "col").trim().toLowerCase().replace(/\s+/g, "_") || "col";
	...
	const handleDeleteColumn = useCallback(
		(colKey) => {
			if (!tableData?.columns?.length || tableData.columns.length <= 1 || !tableId) return;
	...
	const tableColumns = useMemo(() => {
		...
		return tableData.columns.map((col) => ({
			...
			enableSorting: true,
			enableGlobalFilter:
				col.type === "text" || col.type === "url" || col.type === "date",
		}));
				...
				<TableHeader variant="inkgest">
					{table.getHeaderGroups().map((hg) => (
						<TableRow key={hg.id} variant="inkgest" hover={true}>
							<TableHead variant="inkgest" style={{ width: 36, textAlign: "center" }}>
								#
							</TableHead>
					...
					{table.getRowModel().rows.length === 0 ? (
						...
						table.getRowModel().rows.map((row, rowIdx) => (
							<TableRow key={row.id} variant="inkgest" hover={true}>
								...
								<TableCell
									variant="inkgest"
									style={{ width: 44, textAlign: "center", padding: "4px 8px" }}
								>
									<motion.button
			...
			{tableData.sourceUrls?.length > 0 && (
				...
				>
					...
					<span style={{ fontSize: 11, color: T.muted }}>
						Source{tableData.sourceUrls?.length > 1 ? "s" : ""}:
					</span>
