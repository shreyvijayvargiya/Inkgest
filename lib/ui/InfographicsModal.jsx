					...
					{arcs.map((a, i) => (
						...
						>
							...
							<div style={{ flex: 1 }}>
								...
								>
									<span
										style={{ fontSize: 13, color: D.text, fontWeight: 500 }}
									>
										{a.label}
									</span>
									<span
										style={{ fontSize: 13, color: D.muted, fontWeight: 700 }}
									>
										{a.value}%
									</span>
								</div>
								...
								>
									<motion.div
										initial={{ width: 0 }}
										animate={{ width: `${(a.value / total) * 100}%` }}
			...
			<div style={{ position: "relative" }}>
				<div
					style={{
						...
						bottom: 20,
						width: 2,
						background: `linear-gradient(to bottom, ${col}, ${col}22)`,
						borderRadius: 100,
					}}
				/>
				{steps.map((step, i) => (
					...
					>
						<div
							style={{
								...
								border: `2px solid ${palette[i % palette.length]}55`,
								display: "flex",
								alignItems: "center",
			...
			<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
				{[
					...
					},
					{
						...right,
						icon: "✓",
						bg: "rgba(124,157,111,0.12)",
						borderCol: "rgba(124,157,111,0.3)",
						iconColor: "#7C9D6F",
					},
				].map((col, i) => (
					<div
						...
						style={{
							...
							borderRadius: 12,
							padding: "18px 14px",
							border: `1px solid ${col.borderCol}`,
						}}
					>
						<div
							style={{
						...
						{(col.items || []).map((item, j) => (
							...
							>
								...
								<p style={{ fontSize: 12.5, color: D.dim, lineHeight: 1.6 }}>
									{item}
								</p>
							</div>
						))}
...
function StatCard({ data }) {
	...
	return (
		<div
			style={{
				background: `linear-gradient(135deg, #FBF3E4 0%, ${D.card} 100%)`,
				borderRadius: 18,
				padding: "36px 28px",
		...
		>
			<div
				style={{
					...
					height: 180,
					borderRadius: "50%",
					background: `radial-gradient(circle, ${col}28 0%, transparent 70%)`,
				}}
			/>
			<div
				style={{
			...
			<div style={{ position: "relative", zIndex: 1 }}>
				...
				>
					{stat}
					<span style={{ fontSize: 32, color: col }}>{unit}</span>
				</motion.p>
				<p
					style={{
						fontSize: 16,
		...
		>
			<div style={{ position: "absolute", top: 14, left: 20 }}>
				<svg width="52" height="40" viewBox="0 0 52 40">
					<text
						y="40"
						style={{ fontFamily: "", fontSize: 80, fill: col, opacity: 0.2 }}
					>
						{'"'}
					</text>
				</svg>
			...
			<div style={{ position: "relative", zIndex: 1, paddingTop: 16 }}>
				...
				<div style={{ display: "flex", alignItems: "center", gap: 12 }}>
					<div
						style={{ width: 32, height: 2, background: col, borderRadius: 100 }}
					/>
					<div>
						{author && (
							<p style={{ fontSize: 13, fontWeight: 700, color: D.text }}>
								{author}
							</p>
						)}
		...
		>
			...
			<div style={{ position: "relative" }}>
				...
				<div
					style={{
						height: 2,
						background: `linear-gradient(to right, ${col}, ${col}22)`,
						borderRadius: 100,
						marginBottom: 0,
						position: "relative",
					}}
		...
		>
			...
			<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
				{items.map((item, i) => {
					const pct = Math.min(
						100,
						Math.round(((item.value || 0) / (item.max || 100)) * 100),
					);
							...
							>
								<motion.div
									...
									style={{
										height: "100%",
										borderRadius: 100,
										background: `linear-gradient(90deg, ${barCol}, ${barCol}BB)`,
									}}
								/>
							</div>
						</div>
...
function MetricGridCard({ data }) {
	...
	const trendColor = (t) =>
		t === "up" ? "#7C9D6F" : t === "down" ? "#EF4444" : D.muted;
	return (
		<div
			style={{
				background: D.card,
			...
			>
				{metrics.map((m, i) => (
					...
					>
						...
						>
							{m.value}
							<span style={{ fontSize: 14, color: D.muted, marginLeft: 3 }}>
								{m.unit}
							</span>
						</p>
						{m.change && (
...
function InfographicCard({ ig }) {
	switch (ig.type) {
		...
		case "bar":
			return <BarChart data={ig} />;
		case "steps":
			return <StepFlow data={ig} />;
		case "comparison":
			return <ComparisonCard data={ig} />;
		case "stat":
			return <StatCard data={ig} />;
		case "quote":
			return <QuoteCard data={ig} />;
		case "timeline":
			return <TimelineCard data={ig} />;
		case "progress":
			return <ProgressCard data={ig} />;
		case "metric_grid":
			return <MetricGridCard data={ig} />;
		default:
			return (
				<div
					style={{
...
export default function InfographicsModal({
	...
	/* On open: restore saved infographics — never auto-call the API */
	useEffect(() => {
		if (!open) return;
		if (hasAutoGenerated.current) return; // already loaded this session
		hasAutoGenerated.current = true;

		if (savedInfographics.length > 0) {
			// Restore from Firestore — skip API call entirely
			setBatches([savedInfographics]);
		}
		// No auto-generate — user must click "Generate" explicitly
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [open]);

	/* Reset only when modal actually closes so re-open restores from cache */
	useEffect(() => {
		if (!open) {
			// Keep hasAutoGenerated so re-opens don't reload; only reset if
			// we have no infographics (fresh session with nothing to show).
	...
	const handleCopyHtml = (bIdx, iIdx) => {
		const globalIdx =
			batches.slice(0, bIdx).reduce((s, b) => s + b.length, 0) + iIdx;
	...
	const accentOf = (ig) =>
		ig.accentColor || TYPE_DEFAULT_ACCENT[ig.type] || "#C17B2F";
						...
						>
							...
							>
								...
								{loading && (
									<div
										style={{
											...
											alignItems: "center",
											justifyContent:
												allItems.length === 0 ? "center" : "flex-start",
											height: allItems.length === 0 ? "80%" : "auto",
											paddingTop: allItems.length > 0 ? 32 : 0,
											gap: 20,
										}}
									>
										<motion.div
										...
										>
											<motion.div
												animate={{
													width: `${((step + 1) / GEN_STEPS.length) * 100}%`,
												}}
												transition={{ duration: 0.5, ease: "easeOut" }}
												style={{
													height: "100%",
													background: `linear-gradient(90deg, ${T.warm}, #E8A84A)`,
													borderRadius: 100,
												}}
											/>
										</div>
								...
								{!loading && error && allItems.length > 0 && (
									...
									>
										<p style={{ fontSize: 13, color: "#FCA5A5", flex: 1 }}>
											⚠ {error}
										</p>
										<motion.button
											whileHover={{ scale: 1.04 }}
								...
								{!loading && !error && allItems.length === 0 && (
									...
									>
										...
										<div style={{ textAlign: "center", maxWidth: 320 }}>
											...
											>
												AI will analyse your draft and generate visual charts,
												stats, and data cards.
											</p>
										</div>
										<motion.button
											whileHover={{ scale: 1.04, background: T.warm }}
											whileTap={{ scale: 0.97 }}
											onClick={() => generate(false)}
											style={{
												display: "flex",
														...
														return (
															...
															>
																<div
																	...
																	onMouseEnter={(e) => {
																		e.currentTarget.style.boxShadow = `0 8px 28px rgba(0,0,0,0.12), 0 0 0 2px ${accentOf(ig)}55`;
																	}}
																	onMouseLeave={(e) => {
																...
																>
																	...
																	>
																		...
																		<motion.button
																			...
																			whileTap={{ scale: 0.96 }}
																			onClick={() =>
																				handleDownloadPng(batchIdx, iIdx)
																			}
																			title="Download as PNG image (2×)"
																			style={{
																				flex: 1,
																				background: downloading[copyKey]
																					? `${accentOf(ig)}15`
																					: T.base,
																				...
																				justifyContent: "center",
																				gap: 5,
																				cursor: downloading[copyKey]
																					? "default"
																					: "pointer",
																				fontSize: 11,
																				fontWeight: 600,
																				color: downloading[copyKey]
																					? accentOf(ig)
																					: D.muted,
																				transition: "all 0.15s",
																				whiteSpace: "nowrap",
																		...
																		{onInsertToEditor && (
																			<motion.button
																				...
																				whileTap={{ scale: 0.96 }}
																				onClick={() =>
																					handleAddToEditor(batchIdx, iIdx)
																				}
