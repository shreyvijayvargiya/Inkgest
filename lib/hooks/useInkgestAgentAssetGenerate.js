...
import {
	consumeInkAgentResponse,
	getInkAgentEndAssets,
} from "../utils/consumeInkAgentSse";
import { persistAgentResults } from "../utils/persistAgentExecuted";

function mergeTaskProgress(prev, parsed) {
	const key =
		parsed.taskId ??
		parsed.id ??
		parsed.label ??
	...
	const status =
		...
		parsed.stage ||
		parsed.state ||
		(parsed.done ? "done" : "running");
	const next = {
		key,
		label: parsed.label || parsed.taskType || parsed.type || "Task",
		status,
		type: parsed.taskType || parsed.type,
	};
	if (idx >= 0) {
		const copy = [...prev];
...
export function useInkgestAgentAssetGenerate({
	...
	const [multiBlog, setMultiBlog] = useState(false);
		...
		try {
			...
			const { streamedText, finalPayload } = await consumeInkAgentResponse(
				res,
				{
					...
					onPayload: (parsed) => {
						if (parsed.type === "task") {
							setTaskProgress((prev) => mergeTaskProgress(prev, parsed));
						}
						setUrlProgress((prev) => {
							if (!prev.length) return prev;
			...
			setMultiBlog(
				Boolean(finalPayload?.multiBlog) || endAssets.length > 1,
			);
