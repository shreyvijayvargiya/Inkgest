...
export function useGenerateAsset({
	...
	const urlsOk = normalizeUrls(urlInputs).length > 0;
	const promptOk = prompt.trim().length > 0;
	const canSubmit =
		!loading &&
		(assetType === "scrape" ? urlsOk : urlsOk || promptOk);
	...
	const runGenerate = useCallback(async () => {
		...
		if (assetType !== "scrape" && creditRemaining !== Infinity && creditRemaining < n) {
			setError(
		...
		/* URL → raw draft via Next scrape API (1 credit deducted server-side). */
		if (assetType === "scrape") {
			if (!urls.length) {
				setError("Add at least one URL to scrape.");
				return;
			}
			setLoading(true);
			setError(null);
			...
			try {
				...
				const res = await fetch("/api/scrape/url", {
					...
					body: JSON.stringify({ urls, idToken }),
				});
				const data = await res.json().catch(() => ({}));
				if (!res.ok) throw new Error(data.error || "Scrape failed");

				const title = data.title || urls[0] || "Scraped";
				const bodyText = String(data.content || "");
				const words = bodyText.trim().split(/\s+/).filter(Boolean).length;
				const preview = bodyText.slice(0, 180) + (bodyText.length > 180 ? "…" : "");
				const now = new Date();
				...
				const { id } = await createDraft(reduxUser.uid, {
					title,
					preview,
					body: bodyText,
					urls,
				...
				if (!aborted) setError(e?.message || "Scrape failed");
			} finally {
				setLoading(false);
			}
			return;
		...
		const runOne = async (slotIndex, total) => {
			...
			if (tasks.length > 0 && reduxUser?.uid && queryClient) {
				queryClient.invalidateQueries({ queryKey: ["assets", reduxUser.uid] });
			}
