...
function deductCreditsOnEnd(uid, data) {
	if (!uid || !data) return;
	const assetCount = Array.isArray(data.assets)
		? data.assets.length
		: Array.isArray(data.payload?.assets)
			? data.payload.assets.length
			: 0;
	const creditsUsed =
...
export async function validateInkAgentRequest(req) {
	...
	const urlsFromTasks = hasExecuteTasks
		...
		: [];
	const textToCheck = userPrompt || contentStr || urlsFromTasks.join(" ");
