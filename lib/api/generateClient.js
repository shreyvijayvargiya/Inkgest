...
/**
 * POST /generate/:type (Next.js proxy → Hono /generate/:type). Body includes idToken like /api/scrape/url.
...
export async function requestGenerate({
	...
	const url = `${process.env.NEXT_PUBLIC_INKGEST_GENERATE_URL}/generate/${encodeURIComponent(type)}`;
