import { getGenerateUrl } from "../config/generate";
...
/**
 * POST /generate/:type on your Hono app — Bearer Firebase ID token, JSON body.
...
export async function requestGenerate({
	...
	const url = getGenerateUrl(pathType);
	if (!url) {
		throw new Error(
			"Set NEXT_PUBLIC_INKGEST_GENERATE_URL to your Hono API origin (e.g. https://api.example.com)",
		);
	}
